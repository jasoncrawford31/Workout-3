...
# Collapsible section for cardio + measurements
with st.expander("Cardio + Body Measurements"):
    st.markdown("### Cardio")
    cardio_minutes = st.slider("Incline Walk Duration (minutes)", 0, 60, 30)
    calories_burned = round(cardio_minutes * 6.8)
    st.write(f"Estimated Calories Burned: **{calories_burned} kcal**")

    st.markdown("### Body Measurements (inches)")
